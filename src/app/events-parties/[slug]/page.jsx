...
export async function generateMetadata({ params }) {
    const partiesData = await getPartiesBySlug({ slug: params.galleryDetail })
        ...
        return (
            <>
                ...
                <main id="tt-pageContent">
                    <div style={{paddingTop: '0px'}} className="subpages-inner section-marker">
                        <div className="container-fluid">
                            <div className="section-title">
                                {/* <div className="section-title__label">{parseDateWithDayName(new Date(partiesData?.data[0]?.attributes?.eventDate))}</div> */}
                                <h2 className="section-title__text">{partiesData?.data[0]?.attributes?.title}</h2>
                                <div className="title__text-description tt-wide">
                                    {
                                        partiesData.data[0]?.attributes?.longDescription?.map((item, i) => {
